import numpy as np

sbox = []

Rcon = [0x00000000, 0x01000000, 0x02000000,
		0x04000000, 0x08000000, 0x10000000, 
		0x20000000, 0x40000000, 0x80000000, 
		0x1b000000, 0x36000000]
		
def keyExpansion(key):
	#prep w list to hold 44 tuples
	w = [()]*44
	
	#Mengisi 4 huruf / hex pertama dalam matriks 4x4
	for i in range(4):
		w[i] = (key[4*i], key[4*i+1], key[4*i+2], key[4*i+3])
		
	#Memproses rokey per dgn w[0] per kelipatan 4
	for i in range(4, 44):
		#get required previous keywords
		temp = w[i-1]
		word = w[i-4]

		#if multiple of 4 use rot, sub, rcon etc
		if i % 4 == 0:
			x = RotWord(temp)
			y = SubWord(x)
			rcon = Rcon[int(i/4)]

			temp = hexor(y, hex(rcon)[2:]) 
			
		#creating strings of hex rather than tuple
		word = ''.join(word)
		temp = ''.join(temp)
		
		#xor the two hex values
		xord = hexor(word, temp)
		w[i] = (xord[:2], xord[2:4], xord[4:6], xord[6:8])
		
	return w
	
#takes two hex values and calculates hex1 xor hex2
def hexor(hex1, hex2):
	#convert to binary
	bin1 = hextobinary(hex1)
	bin2 = hextobinary(hex2)
	
	#calculate
	xord = int(bin1, 2) ^ int(bin2, 2)
	
	#cut prefix
	hexed = hex(xord)[2:]
	
	#leading 0s get cut above, if not length 8 add a leading 0
	if len(hexed) != 8:
		hexed = '0' + hexed
		
	return hexed
	
#Mengonversi nilai hexadesimal ke biner
def hextobinary(hex):
	return bin(int(str(hex), 16))

	
#takes from 1 to the end, adds on from the start to 1
def RotWord(word):
	return word[1:] + word[:1]
		
		
#Subtitusi S-Box
def SubWord(word):
	sWord = ()
	
	#loop throug the current word
	for i in range(4):
		
		#check first char, if its a letter(a-f) get corresponding decimal
		#otherwise just take the value and add 1
		if word[i][0].isdigit() == False:
			row = ord(word[i][0]) - 86
		else:
			row = int(word[i][0])+1

		#repeat above for the seoncd char
		if word[i][1].isdigit() == False:
			col = ord(word[i][1]) - 86
		else:
			col = int(word[i][1])+1
		
		#get the index base on row and col (16x16 grid)
		sBoxIndex = (row*16) - (17-col)
		
		#get the value from sbox without prefix
		piece = hex(sbox[sBoxIndex])[2:]
		
		#check length to ensure leading 0s are not forgotton
		if len(piece) != 2:
			piece = '0' + piece
		
		#form tuple
		sWord = (*sWord, piece)
		
	#return string
	return ''.join(sWord)

#Menampilkan nilai baris roundkey: w0 = 0f 15 71 c9
def prettyPrint(rkey):
	print("\n\nKeywords: \n")

	for i in range(len(rkey)):
		print("\nRound Key ke-" + str(i), "=", rkey[i])
	
def expand(cipherkey):
 
	key = np.transpose(cipherkey)
	#print(key)
	key = key.flatten().tolist()
	#print(key)
	key = [hex(i)[2:] for i in key]
	#print(key)
	#expand key
	w = keyExpansion(key)
	
	#Menampilkan Kunci
	#print("\nKey provided: " + "".join(key))
	
	for i in range(len(w)):
		w[i] = list(w[i])
  
	rkeys = [np.transpose(w[j*4:j*4+4]) for j in range(len(w)//4)]
	
	# Mengonversi setiap nilai heksadesimal ke desimal
	rkey = [[[int(hex_value, 16) for hex_value in row] for row in matrix] for matrix in rkeys]

	prettyPrint(rkey)
 
 
	#print("Total Kunci Ronde : ",len(rkey))
 
	return rkey
	

def addRoundKey(state, roundKey):
    for i in range(4):
        for j in range(4):
            	state[i][j] = state[i][j] ^ roundKey[i][j]

 
 
